...
class Settings(BaseSettings):
    """Gens settings."""
    gens_db: MongoDsn = Field("mongodb://mongodb:27017/gens", description="Connection to Gens mongo database.")

    # For scout integration
    scout_db: MongoDsn = Field("mongodb://mongodb:27017/scout", description="Connection to Gens mongo database.")
    scout_url: HttpUrl = Field("http://localhost:8000", description="Base URL to Scout.")
    ...
    def check_oauth_opts(self):
        ...
        if self.authentication == AuthMethod.OAUTH:
            ...
            if not all(checks):
                raise ValueError("OAUTH require you to configure client_id, secret and discovery_url")
        return self
    ...
    def check_mongodb_connections(self):
        ...
        self.gens_dbname = self.gens_dbname if conn_info['database'] is None else conn_info['database']

        # scout 
        conn_info = parse_uri(str(self.scout_db))
        self.scout_dbname = self.scout_dbname if conn_info['database'] is None else conn_info['database']
