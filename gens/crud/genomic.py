...
def get_chromosomes(db: Database[Any], genome_build: GenomeBuild) -> list[ReducedChromInfo]:
    """Gets the size in base pairs of a chromosome."""
    chrom_data = db[CHROMSIZES_COLLECTION].find({"genome_build": genome_build}, {"_id": False, "chrom": True, "size": True})
...
def get_chromosome_info(
    ...
    if chrom_data is None:
        LOG.warning(
            "Could not find data for chromosome %s in DB; genome_build: %d", chromosome, genome_build
        )
        return None
