...
describe("Test if point is within element", () => {
  ...
  test("test point within element bbox, xy", () => {
    expect(
      isWithinElementBbox(element, { x: 20, y: 5 }),
    ).toBeTruthy();
  });
  test("test point is outside element bbox", () => {
    expect(
      isWithinElementBbox(element, { x: 20, y: 15 }),
    ).toBeFalsy();
  });
  test("test point is on element bbox edge", () => {
    expect(
      isWithinElementBbox(element, { x: 10, y: 10 }),
    ).toBeFalsy();
    expect(
      isWithinElementBbox(element, { x: 50, y: 10 }),
    ).toBeFalsy();
  });
});
...
describe("Test getVisibleYCoordinates", () => {
  test("test element higher than minHeight", () => {
    ...
    const resp = getVisibleYCoordinates(element, 4);
  ...
  test("test element shorter than minHeight", () => {
    ...
    const minHeight = 20;
    const resp = getVisibleYCoordinates(element, minHeight);
