...
export function updateVisibleElementCoordinates({
  element,
  screenPosition,
  scale,
}: {
...
function teardownTooltips(track) {
  while (track.geneticElements.length) {
    ...
    track.trackContainer
      .querySelector(`#${element.tooltip.tooltip.id}`)
      .remove();
  }
}
...
export function initTrackTooltips(track: BaseAnnotationTrack) {
  ...
  // when mouse is leaving track
  track.trackContainer.addEventListener("mousemove", (e) => {
    tooltipHandler(e, track);
  });
  ...
  track.drawOffScreenTrack = async ({
    startPos,
    endPos,
    maxHeightOrder,
    data,
    ...
    await oldDrawOffscreenTrack.call(track, {
      startPos,
      endPos,
      maxHeightOrder,
      data,
