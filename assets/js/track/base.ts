...
export function calculateOffscreenWindowPos({ start, end, multiplier }) {
  ...
  return {
    start: Math.round(start - padding),
    end: Math.round(end + padding),
  };
}
...
export class BaseAnnotationTrack {
  ...
  tracksYPos(heightOrder) {
    return (
      this.yPos + (heightOrder - 1) * (this.featureHeight + this.featureMargin)
    );
  }
  ...
  async drawTrack({
    ...
    //  3. right expansion
    if (!this.trackData || this.trackData.chromosome !== chrom || forceRedraw) {
