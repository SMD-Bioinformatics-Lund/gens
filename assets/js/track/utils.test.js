...
describe("Test if point is within element", () => {
  ...
  test("test point within element bbox, xy", () => {
    expect(
      isWithinElementBbox({ element, point: { x: 20, y: 5 } }),
    ).toBeTruthy();
  });
  test("test point is outside element bbox", () => {
    expect(
      isWithinElementBbox({ element, point: { x: 20, y: 15 } }),
    ).toBeFalsy();
  });
  test("test point is on element bbox edge", () => {
    expect(
      isWithinElementBbox({ element, point: { x: 10, y: 10 } }),
    ).toBeFalsy();
    expect(
      isWithinElementBbox({ element, point: { x: 50, y: 10 } }),
    ).toBeFalsy();
  });
});
