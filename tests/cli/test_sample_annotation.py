...
def test_load_sample_annotation_updates_existing(reload_cli, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    load_mod = reload_cli
    load_sample_annotation_cmd = load_mod.sample_annotation
    # db = DummyDB()
...
def test_update_sample_annotation_track_modifies_collection(reload_cli) -> None:
    import importlib
    ...
    # "mongomock" stub does not provide ``matched_count``
    # setattr(mongomock.UpdateResult, "matched_count", 1)
...
def test_delete_sample_annotation_track_removes_document(reload_cli) -> None:
    import importlib
    load_mod = importlib.import_module("gens.crud.sample_annotations")
    client = my_mongomock.MongoClient()
    db = client.get_database("test")
